...
global TTU_CoordMode_ToolTip := ""
global TTU_CoordMode_Mouse := ""

;ToolTipをマウスではなくキャレット上に出す

ToolTipOnCarret(message, waittime := -1440) {
    TTU_GetCoordMode()

    GetCaretPos(&X, &Y)
    ToolTip(message, X, Y + 20)
    SetTimer(TTU_ToolTipTimer, waittime)

}

;ToolTipをマウス上に出す（たまにToolTipがマウスの上に出てくれないため位置指定）

ToolTipOnMouse(message, waittime := -1440) {
    TTU_GetCoordMode()
...
;CoordModeのデフォルトはScreenだがClientになっていることがある　再設定

TTU_GetCoordMode() {
    global TTU_CoordMode_ToolTip, TTU_CoordMode_Mouse

    TTU_CoordMode_ToolTip := A_CoordModeToolTip
    TTU_CoordMode_Mouse := A_CoordModeMouse

    if TTU_CoordMode_ToolTip != "Screen" {
        CoordMode "ToolTip", "Screen"
    }

    if TTU_CoordMode_Mouse != "Screen" {
...
TTU_SetCoordMode() {
    ...
    CoordMode "ToolTip", TTU_CoordMode_ToolTip
    CoordMode "Mouse", TTU_CoordMode_Mouse
}

;;; GetCaretPosの参考先　This function is based on a post from the AutoHotkey forum
